...
with sd15_info:
    ...
    with st.spinner("Loading Lora Metadata...."):
        try:
            lora_metadata = {
                "name": selected_lora,
                "lora_path": enumerate_models(appSettings.config_parameters.loras.sd15.path)
                .get(selected_lora)
                .absolute(),
                "metadata": read_safetensors_header(
    ...
    with st.spinner("Loading VAE Metadata...."):
        try:
            vae_metadata = {
                ...
                "vae_path": enumerate_models(appSettings.config_parameters.vae.sd15.path)
                .get(selected_vae)
                .absolute(),
                "metadata": read_safetensors_header(
...
with sd15_merger:
    ...
    if do_advanced_merge and can_merge:
        try:
            # Execute merge
            output_path = (
                f"{appSettings.config_parameters.checkpoints.sd15.path}/{output_name}"
                if save_output
                else None
            )
            ...
            with st.spinner(f"🔄 Executing {merge_method.upper()} merge..."):
                ...
                if output_path:
                    result = (
                        st.session_state.merge_pipeline.merge_for_pipeline_generator(
    ...
    if target_models:
        ...
        if st.button(
            ...
            try:
                ...
                output_dir = (
                    PosixPath(appSettings.config_parameters.checkpoints.sd15.path)
                    / output_subdir
                )
...
with recipe_builder:
    ...
    # Add new step
    with st.expander(
        "➕ Add Merge Step", expanded=len(st.session_state.current_recipe["steps"]) == 0
    ):
        ...
        with exec_pane:
            if st.button(
                ...
                try:
                    ...
                    # Execute recipe
                    with st.spinner(
                        f"🧪 Executing {len(st.session_state.current_recipe['steps'])}-step recipe..."
                    ):
                        success = st.session_state.merge_pipeline.execute_merge_recipe(
                            recipe, output_path
                        )
                                ...
                                st.json(
                                    {
                                        ...
                                        "steps_count": len(
                                            st.session_state.current_recipe["steps"]
                                        ),
