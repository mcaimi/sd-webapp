...
try:
    ...
    with st.spinner("** LOADING INTERFACE... **"):
        ...
        from libs.stablediffusion.sdxl_merge import (
            SDXLMergePipeline,
            MergeConfig,
            MergeMethod,
        )
...
with sdxl_info:
    ...
    with st.spinner("Loading Model Metadata...."):
        try:
            ...
            model_info.json({"exception": f"{e}"}, expanded=False)
    ...
    with st.spinner("Loading Lora Metadata...."):
        try:
            lora_metadata = {
                "name": selected_lora,
                "lora_path": enumerate_models(appSettings.config_parameters.loras.sdxl.path)
                .get(selected_lora)
                .absolute(),
                "metadata": read_safetensors_header(
            ...
            lora_info.json({"exception": f"{e}"}, expanded=False)
    ...
    selected_vae = vae_selection.selectbox(
        ...
        options=enumerate_models(appSettings.config_parameters.vae.sdxl.path).keys(),
        index=0,
    )
    ...
    with st.spinner("Loading VAE Metadata...."):
        try:
            vae_metadata = {
                ...
                "vae_path": enumerate_models(appSettings.config_parameters.vae.sdxl.path)
                .get(selected_vae)
                .absolute(),
                "metadata": read_safetensors_header(
            ...
            vae_info.json({"exception": f"{e}"}, expanded=False)
...
with sdxl_merger:
    ...
    with st.expander(label="🔍 Merge Compatibility Check", expanded=True):
        ...
        ckpt_path_a = enumerate_models(
            appSettings.config_parameters.checkpoints.sdxl.path
        ).get(selected_checkpoint_a)
        ckpt_path_b = enumerate_models(
            appSettings.config_parameters.checkpoints.sdxl.path
        ).get(selected_checkpoint_b)
        ...
        try:
            ...
            with st.spinner("Validating compatibility..."):
                ...
                target_weights, _ = load_checkpoint_dict(
                    ckpt_path_b, device=get_gpu()[0]
                )
    ...
    if do_advanced_merge and can_merge:
        try:
            # Execute merge
            output_path = (
                f"{appSettings.config_parameters.checkpoints.sdxl.path}/{output_name}"
                if save_output
                else None
            )
            ...
            with st.spinner(f"🔄 Executing {merge_method.upper()} merge..."):
                ...
                config = MergeConfig(
                    ...
                    progress_callback=lambda p: merge_progress.progress(p)
                    if show_progress
                    else None,
                )
                ...
                if output_path:
                    result = (
                        st.session_state.xl_merge_pipeline.merge_for_pipeline_generator(
    ...
    with st.expander("🗄️ Cache Information"):
        cache_info = st.session_state.xl_merge_pipeline.get_cache_info()
        st.json(cache_info)

with xl_batch_merger:
    st.markdown("### 📦 Batch Model Processing")
    st.markdown(
        "*Create Many Merges between a Base Model and Several Target Models in an automated Batch*"
    )
    ...
    with base_info_col:
        base_path = enumerate_models(
            appSettings.config_parameters.checkpoints.sdxl.path
        ).get(selected_base)
        base_metadata = read_safetensors_header(base_path)
        st.json(
            {"base_model": selected_base, "metadata": base_metadata}, expanded=False
        )
    ...
    if target_models:
        ...
        if st.button(
            "🔄 Start Batch Merge", type="primary", disabled=len(target_models) == 0
        ):
            try:
                ...
                output_dir = (
                    PosixPath(appSettings.config_parameters.checkpoints.sdxl.path)
                    / output_subdir
                )
                ...
                with st.spinner(f"🔄 Processing {len(target_models)} models..."):
                    ...
                    results = []
                    for i, (target_model, target_path) in enumerate(
                        zip(target_models, target_paths)
                    ):
                        # Update progress
                        progress = (i + 1) / len(target_models)
                        progress_bar.progress(progress)
                        status_text.text(
                            f"Processing {target_model} ({i + 1}/{len(target_models)})"
                        )
                ...
                if successful > 0:
                    st.success(
                        f"✅ Batch processing completed! {successful} models merged successfully."
                    )
                else:
...
with xl_recipe_builder:
    ...
    with st.expander(
        "➕ Add Merge Step",
        expanded=len(st.session_state.xl_current_recipe["steps"]) == 0,
    ):
        ...
        with merge_step_settings:
            step_alpha = st.slider(
                "Step Strength", 0.0, 1.0, 0.5, 0.05, key="new_step_alpha"
            )
            step_progress = st.checkbox(
                "Show Progress", value=True, key="new_step_progress"
            )
        ...
        if st.button("➕ Add Step"):
            ...
            st.session_state.xl_current_recipe["steps"].append(new_step)
            st.rerun()
    ...
    if st.session_state.xl_current_recipe["steps"]:
        ...
        for i, step in enumerate(st.session_state.xl_current_recipe["steps"]):
            with st.container(border=True):
                ...
                with step_action_col:
                    if st.button(
                        "🗑️ Delete Step", key=f"delete_step_{i}", help="Delete step"
                    ):
        ...
        with exec_pane:
            if st.button(
                "🧪 Execute Recipe",
                type="primary",
                disabled=len(st.session_state.xl_current_recipe["steps"]) == 0,
            ):
                try:
                    ...
                    # Output path
                    output_path = f"{appSettings.config_parameters.checkpoints.sdxl.path}/{recipe_output}"

                    # Execute recipe
                    with st.spinner(
                        f"🧪 Executing {len(st.session_state.xl_current_recipe['steps'])}-step recipe..."
                    ):
                        success = (
                            st.session_state.xl_merge_pipeline.execute_merge_recipe(
                                recipe, output_path
                            )
                        )
                        ...
                        if success:
                                ...
                                st.json(
                                    {
                                        ...
                                        "steps_count": len(
                                            st.session_state.xl_current_recipe["steps"]
                                        ),
                            ...
                            st.error(
                                "❌ Recipe execution failed. Check console for details."
                            )
            ...
            if st.button("🗑️ Clear Recipe"):
                st.session_state.xl_current_recipe = {
                    "base_model": recipe_base,
                    "steps": [],
                }
                st.rerun()
